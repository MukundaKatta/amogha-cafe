...
export function finalizeAddToCart(itemName, price, spiceLevel, addons, btnEl) {
    ...
    if (window._ariaAnnounce) {
        var qty = (existingItem ? existingItem.quantity : 1);
        window._ariaAnnounce(itemName + ' added to cart. Quantity: ' + qty);
    }
...
export function updateFloatingCartBar() {
    ...
    var subtotal = cart.reduce(function(t, i) {
        var addonTotal = (i.addons || []).reduce(function(s, a) { return s + a.price; }, 0);
        return t + (i.price + addonTotal) * i.quantity;
    }, 0);
    if (count === 0) {
...
export function clearCart() {
    if (confirm('Are you sure you want to clear your cart?')) {
        ...
        unlockScroll();
        if (window._ariaAnnounce) window._ariaAnnounce('Cart cleared');
    }
}
