...
app.post('/auth/kiosk-login', async function(req, res) {
    try {
        ...
        snap.forEach(function(doc) {
            var d = doc.data();
            // Support both hashed and plaintext passwords (migration path)
            var isMatch = d.passwordHash
                ? timingSafeEqual(hashValue(password, 'kiosk'), d.passwordHash)
                : timingSafeEqual(password, d.password || '');
            if (isMatch) {
        ...
        shopsSnap.forEach(function(doc) {
            var d = doc.data();
            var pinMatch = d.adminPinHash
                ? timingSafeEqual(hashValue(password, 'admin'), d.adminPinHash)
                : timingSafeEqual(password, d.adminPin || '');
            if (doc.id === username && pinMatch) {
...
app.post('/auth/delivery-login', async function(req, res) {
    try {
        ...
        // Support both hashed and plaintext PINs (migration path)
        var pinMatch = d.pinHash
            ? timingSafeEqual(hashValue(pin, 'delivery'), d.pinHash)
            : timingSafeEqual(pin, d.pin || '');
        if (!pinMatch) {
